...
public class PizzaOrderServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// req에서 꺼내올 값도 없으니 별도의 처리 없이 바로 JSP 로 요청 위임해보자
		
		// RequestDispatcher : 요청 발송자
		// -> 지정된 JSP로 요청 정보(req), 응답 정보(resp)fmf
		//	  전송(발송)하는 역할
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// *** POST 방식으로 하니 한글 깨짐 문제가 발생한다 ***
		
		// - 왜? 데이터 전달 방식의 차이 때문에.
		// GET : 주소(URL)를 통해서 데이터 전달
		//		 이 때, 문자 인코딩은 제출된 HTML 파일의 문자 인코딩(charset을 따름)
		
		// POST : HTTP Body를 통해서 데이터 전달
		//		  이 때, 문자 인코딩은 서버의 기본 문자 인코딩을 따름
		//		  우리가 사용중인 서버(Tomcat) -> ISO-8859-1 이 기본 문자 인코딩임
		...
		System.out.println("pizza : " + pizza); // 피즈 피자-8000
		System.out.println("size : " + size);
		System.out.println("amount : " + amount);
		
		// "-" 을 기준으로 피자 이름, 가격 나누기
		String[] arr = pizza.split("-"); // "-" 구분자로 쪼개서 String 배열로 반환
		// arr == {"치즈 피자", "8000"}
		...
		// 하지만 Servlet에서 만든 변수인 pizzaName, price는 없다!
		
		// [해결 방법]
		// req에 속성(Attribute)로 추가하면 JSP에서 꺼내쓸 수 있다!
