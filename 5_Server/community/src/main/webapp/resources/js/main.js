...
function loginValidate(){ // 로그인 유효성 검사
    ...
    if(inputPw.value.trim().length == 0){
        ...
        alert("비밀번호를 입력해주세요.");
        inputPw.focus(); // 이메일 input에 초점 맞추기
        inputPw.value = "";
        return false; // 기본 이벤트 제거를 위해 false 반환
    }
}
...
if(saveId != null){
    ...
    if(this.checked){ // 체크박스가 체크된 경우
        const str = "개인정보 보호를 위해 개인 PC에서의 사용을 권장합니다. 개인 PC가 아닌 경우 취소를 눌러주세요."
...
document.getElementById("selectBtn").addEventListener("click", function(){
    ...
    $.ajax({
        ...
        success : function(member){
            console.log(member); // JS 객체 형태 문자열(dataType 미지정 시)
...
(function(){
    ...
    selectAll(); // 함수 호출 -> 회원 목록을 사이트 들어오자마자 바로 조회하기 위함

    // window.setInterval(함수, 딜레이(ms))
    // -> 딜레이 시간 만큼 먼저 지연 후 함수 실행 -> 지연 -> 실행... 반복

    window.setInterval(selectAll, 10000) // 10초
    // 함수 이름만 작성 -> 함수 코드 대입됨
    // -> 10초 마다 selectAll(회원 전체 조회하는 함수) 실행
})();