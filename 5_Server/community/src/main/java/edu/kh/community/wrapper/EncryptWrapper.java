...
public class EncryptWrapper extends HttpServletRequestWrapper{
	...
	// 생성자가 하나라도 작성되어 있으면 컴파일러가 기본적으로 기본생성자 추가 안해줌
	...
	public String getParameter(String name) {
		...
		switch(name) {
		case "memberPw" : case "inputPw" : 
			value = getSha512(super.getParameter(name)); break;
		
		// 암호화 되는 경우가 아니라면 기존 getParameter() 메소드의 형태를 유지한다
	...
	private String getSha512(String pw) {
		...
		try {
			...
			// 2. SHA-512 방식의 해시 함수를 수행할 수 있는 객체를 얻어오기
			...
			// SHA-512 해시 함수가 존재하지 않을 때 예외가 발생
			e.printStackTrace();
		}
