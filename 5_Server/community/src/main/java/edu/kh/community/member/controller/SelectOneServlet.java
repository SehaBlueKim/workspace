...
public class SelectOneServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// 해당 참조형 변수의 toString() 메소드를 자동으로 호출해서 출력해버림
			
			// java 객체 -> JSON(javascript 객체 형태의 문자열) -> javascript 객체 형태로 내보내주기
			//				"{ K : V }"
			// java에서 javascript로 데이터를 넘겨주고 싶은데 서로 타입이 안맞음
			// 어떡하지? -> javascript 형태의 문자열인 JSON으로 만들어서 넘겨줄게 
			
			// 1번 방법) JSON 직접 작성 -> 오타 많이 남, 작성하기 번거로움
			// String str = "{\"memberEmail\": \"" + member.getMemberEmail() + "\"}";
			// System.out.println(str);
			
			// 2번 방법) JSON-Simple 라이브러리에서 제공하는 JSONObject 사용 (JSON 라이브러리 필요)
			/*if(member != null) {
			...
			*/
			...
			// 3번 방법) GSON 라이브러리를 이용한 Java 객체 -> JSON 변환 (GSON 라이브러리 필요)
