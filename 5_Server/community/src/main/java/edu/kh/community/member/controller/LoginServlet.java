...
public class LoginServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// 이메일, 비밀번호 일치하는 회원을 조회하는 서비스 호출 후 결과 반환 받기
			Member loginMember = service.login(mem);
			
			// 로그인 성공/실패에 따른 처리
			
			// *** 로그인이란 무엇인가... ***
			// ID/PW가 일치하는 회원 정보를 Session Scope에 세팅하는 것을 말함
			...
			if(loginMember != null) { // 로그인 성공
				...
				// 회원의 정보를 Session에 세팅해보자
				session.setAttribute("loginMember", loginMember);
				
				
				// 특정 시간 동안 요청이 없으면 세션 만료되게 해보자
				...
				/* Cookie : 클라이언트(즉, 브라우저)에서 관리하는 파일
				 * 
				 * - 특정 주소 요청 시 마다
				...
				// -> /community로 시작되는 주소에서만 해당 쿠키를 적용
				
				// 응답 객체를 이용해서 클라이언트로 쿠키 전달해줘야함
				resp.addCookie(c); // 코드가 해석되는 순간 바로 전달됨
			...
			// 클라이언트 요청 -> 서버에서 요청 처리(Servlet) -> 응답 화면 만들어줘(JSP에게 위임)
			
			// 1. forward(요청 위임)
			// - Servlet으로 응답화면 만들기 불편
			// -> JSP로 req, resp 객체를 위임하여
			//	  요청에 대한 응답 화면을 대신 만들었다.
			
			// ** 화면이 변경돼도 요청 주소가 유지된다.
			
			// ex) 아이스 아메리카노 주세요~ ->   주문 받음   -> 바리스타가 만든 아.아
			//		 클라이언트 요청			캐셔(Servlet)		 응답 결과(JSP)
			...
			//	    롯데시네마
			// ex) 팝콘 주세요~ -> 팝콘 파는 위치를 알려줌 -> (클)팝콘 파는 곳으로 이동, 팝콘 주세요~
			//	  클라이언트 요청     캐셔(Servlet)			클라이언트의 다른 주소로 재요청
			
			resp.sendRedirect(req.getContextPath());
			// req.getContextPath() : 해당 프로젝트의 최상위 주소를 얻어옴
