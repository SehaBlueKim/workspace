...
-- 3) 부서코드가 노옹철사원과 같은 소속의 직원 이름, 부서코드 조회
--> 1,2 단계를 하나의 쿼리로 작성!
...
-- 노옹철 사원의 급여보다 많이 받는 지원의
-- 사번, 이름, 부서코드, 직급코드, 급여 조회
SELECT EMP_ID, EMP_NAME ,DEPT_CODE, JOB_CODE ,SALARY
FROM EMPLOYEE
WHERE SALARY > (SELECT SALARY FROM EMPLOYEE WHERE EMP_NAME='노옹철');

-- 부서 별(부서가 없는 사람도 포함) 급여의 합계 중 가장 큰 부서의
-- 부서명, 급여 합계를 조회
...
-- 2) 부서 별 급여 합이 17700000인 부서의 부서명, 급여 합 조회
...
-- 3) 사수에 해당하는 직원에 대한 정보 조회(이 때, 구분은 '사수'로)
...
-- 4) 일반 직원에 해당하는 사원들 정보 조회(이 때, 구분은 '사원'으로)
...
-- 5) 3, 4의 조회 결과를 하나로 합치고 싶다 --> SELECT절 SUBQUERY를 이용
...
-- A > ANY B : A가 B의 여러 결과 중에서 하나라도 크면 TRUE
			--> A가 B의 최소값 보다 큰 경우
			
-- A < ANY B : A가 B의 여러 결과 중에서 하나라도 작으면 TRUE
			--> A가 B의 최대값 보다 작은 경우
			
-- 차장 직급의 급여의 가장 큰 값 보다 많이 받는 과장 직급 직원의
-- 사번, 이름, 직급명, 급여를 조회
-- > ALL 혹은 < ALL 연산자 사용

-- 1) 과장 직급 직원의 사번, 이름, 직급명, 급여
SELECT EMP_ID , EMP_NAME, JOB_NAME, SALARY 
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '과장';
...
WHERE LOCATION_ID = (SELECT LOCAL_CODE
					FROM LOCATION
					WHERE NATIONAL_CODE = 'KO'); -- 5행

-- 3) EMPLOYEE 테이블에서 위의 결과와 동일한 DEPT_CODE를 가지는 사원 조회
...
-- 1) 급여를 200만, 600만 받는 직원 조회 (200만, 600만이 평균 급여라고 가정)
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE 
...
SELECT AVG(SALARY) FROM EMPLOYEE E2
				WHERE E2.JOB_CODE = 'J3';
			
-- 부서별 입사일이 가장 빠른 사원의
-- 사번, 이름, 부서명(NULL인 경우 '소속 없음'), 직급명, 입사일을 조회
-- 입사일이 빠른 순으로 조회
-- 단, 퇴사한 직원은 제외
...
-- 메인쿼리 안에 작성하는 또 다른 SELECT문

SELECT EMP_NAME, SALARY, (SELECT AVG(SALARY) FROM EMPLOYEE)
FROM EMPLOYEE ;
...
SELECT EMP_ID, EMP_NAME, JOB_NAME , 
		FLOOR( MONTHS_BETWEEN (SYSDATE, TO_DATE(19 || SUBSTR(EMP_NO, 1, 6), 'RRRRMMDD') ) / 12) 나이, 
		TO_CHAR( SALARY * (1 + NVL(BONUS,0)) * 12, 'L999,999,999') "보너스 포함 연봉"
FROM EMPLOYEE E1
JOIN JOB J ON(E1.JOB_CODE = J.JOB_CODE)
WHERE FLOOR( MONTHS_BETWEEN (SYSDATE, TO_DATE(19 || SUBSTR(EMP_NO, 1, 6), 'RRRRMMDD') ) / 12) -- 나이
	= (SELECT MIN( FLOOR( MONTHS_BETWEEN (SYSDATE, 
		TO_DATE(19 || SUBSTR(EMP_NO, 1, 6), 'RRRRMMDD') ) / 12))
		FROM EMPLOYEE E2
		WHERE E1.JOB_CODE = E2.JOB_CODE)
ORDER BY 나이 DESC;

SELECT EMP_ID, EMP_NAME, JOB_NAME, 나이, 보너스 포함 연봉
FROM EMPLOYEE E1
JOIN JOB J ON (E1.JOB_CODE = J.JOB_CODE)
WHERE 나이 = (직급별 가장 어린 직원)
ORDER BY 나이 DESC;

-- 직급별 가장 어린 직원
SELECT MIN( FLOOR( MONTHS_BETWEEN (SYSDATE, 
			TO_DATE(19 || SUBSTR(EMP_NO, 1, 6), 'RRRRMMDD') ) / 12)) 나이
FROM EMPLOYEE
WHERE JOB_CODE = 'J3';
