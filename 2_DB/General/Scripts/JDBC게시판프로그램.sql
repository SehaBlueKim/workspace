...
CREATE TABLE MEMBER (
	...
	MEMBER_GENDER CHAR(1) CHECK(MEMBER_GENDER IN('F', 'M')),
...
-- 아이디 중복 검사
SELECT COUNT(*) FROM MEMBER WHERE MEMBER_ID = 'USER01' AND SECESSION_FL = 'N'; -- 탈퇴 안한 계정만 조회
...
-- 게시글 목록 조회 + 댓글 개수까지(상관 서브쿼리 + 스칼라 서브쿼리)
...
-- 게시글 샘플 데이터
INSERT INTO BOARD 
VALUES(SEQ_BOARD_NO.NEXTVAL, '샘플 게시글 1', '샘플1 내용입니다.', DEFAULT, DEFAULT, 41);

INSERT INTO BOARD 
VALUES(SEQ_BOARD_NO.NEXTVAL, '샘플 게시글 2', '샘플2 내용입니다.', DEFAULT, DEFAULT, 41);

INSERT INTO BOARD 
VALUES(SEQ_BOARD_NO.NEXTVAL, '샘플 게시글 3', '샘플3 내용입니다.', DEFAULT, DEFAULT, 41);


-- 댓글 샘플 데이터 삽입
INSERT INTO REPLY
VALUES(SEQ_REPLY_NO.NEXTVAL, '샘플1의 댓글1', DEFAULT, 41, 1);

INSERT INTO REPLY
VALUES(SEQ_REPLY_NO.NEXTVAL, '샘플1의 댓글2', DEFAULT, 41, 1);

INSERT INTO REPLY
VALUES(SEQ_REPLY_NO.NEXTVAL, '샘플1의 댓글3', DEFAULT, 41, 1);
...
-- 댓글 목록에서 최근에 작성한 댓글은 제일 위로 가게? 아래로 가게? 
--								SNS			카페, 커뮤니티
...
--> 기본적으로 삭제 불가
--> 삭제 옵션을 추가하면 가능
-- ON DELET SET NULL(자식 컬럼 NULL값으로 바꿔줌) / ON DELETE CASCADE(참조하던 자식 행도 삭제)

-- 제약조건은 ALTER(변경) 없음 -> 삭제 후 다시 추가

-- 기존 REPLY 테이블 FK 제약조건 삭제
ALTER TABLE REPLY DROP CONSTRAINT SYS_C007539;
...
-- ORA-02292: 무결성 제약조건(MEMBER_KSH.SYS_C007539)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
INSERT INTO BOARD 
VALUES(SEQ_BOARD_NO.NEXTVAL, '새로운 게시글', '안녕하세요', DEFAULT, DEFAULT, 41);
...
FROM BOARD B
JOIN MEMBER USING(MEMBER_NO)
-- WHERE BOARD_TITLE LIKE '%키워드%' -- 제목 검색

-- WHERE BOARD_CONTENT LIKE '%키워드%' -- 내용 검색

WHERE (BOARD_TITLE LIKE '%스트링%' OR
		BOARD_CONTENT LIKE '%스트링%') -- 제목 + 내용 검색

-- WHERE MEMBER_NM LIKE '%키워드%' -- 작성자 검색
ORDER BY BOARD_NO DESC;
