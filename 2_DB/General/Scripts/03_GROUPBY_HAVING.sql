...
SELECT DEPT_CODE FROM EMPLOYEE ; -- 결과값 23행

SELECT SUM(SALARY) FROM EMPLOYEE ; -- 결과값 1행

SELECT DEPT_CODE, SUM(SALARY)
FROM EMPLOYEE 
GROUP BY DEPT_CODE ; -- 부서별로 그룹을 지어 각 그룹의 합계를 조회


-- EMPLOYEE 테이블에서
-- 부서코드, 부서 별 급여의 합계, 부서 별 급여의 평균(정수처리), 인원수를 조회하고
-- 부서코드 순으로 정렬
...
-- EMPLOYEE 테이블에서
-- 부서코드와 부서 별 보너스를 받는 사원의 수를 조회하고
-- 부서코드 순으로 정렬
...
FROM EMPLOYEE 
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남자', 2, '여자') -- 별칭 사용 불가
ORDER BY "인원 수" DESC ;
...
-- EMPLOYEE 테이블에서 부서 별로 같은 직급인 사원의 급여 합계 조회
-- 부서 코드 오름차순으로 정렬
...
FROM EMPLOYEE 
GROUP BY DEPT_CODE 
HAVING AVG(SALARY) >= 3000000 -- 부서 별 평균 급여가 300만 이상
ORDER BY 1;

SELECT DEPT_CODE, FLOOR(AVG(SALARY))
FROM EMPLOYEE 
WHERE SALARY  >= 3000000 -- 급여가 300만 이상인 직원의 부서 별 급여 평균
GROUP BY DEPT_CODE 
ORDER BY 1;
...
-- EMPLOYEE 테이블에서 각 부서 별 가장 높은 급여, 가장 낮은 급여를 조회하여
-- 부서코드 오름차순 정렬
...
-- EMPLOYEE 테이블에서 각 직급 별 보너스를 받는 사원의 수를 조회하여
-- 직급코드 오름차순 정렬
...
-- EMPLOYEE 테이블에서
-- 부서 별 70년대생의 급여 평균이 300만 이상인 부서를 조회하여
-- 부서코드 내림차순 정렬
