...
-- 그룹(GROUP) 함수 : N개의 값을 읽어서 1개의 결과를 반환
...
-- CEIL(숫자 | 컬럼명) : 올림
-- FLOOR(숫자 | 컬럼명) : 내림
--> 둘 다 소수점 첫째 자리에서 올림/내림 처리 (자리수 지정 안됨)

SELECT 123.5, CEIL(123.52), FLOOR(123.52) FROM DUAL;
...
CEIL (MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) || '개월' AS "근무 개월 수", 
CEIL (MONTHS_BETWEEN(SYSDATE, HIRE_DATE)/12) || '년 차' AS "근무 년차"
FROM EMPLOYEE;
...
SELECT TO_CHAR(1234, '99999') FROM DUAL; -- 숫자 5칸을 오른쪽으로 정렬
SELECT TO_CHAR(1234, '00000') FROM DUAL; -- 숫자 5칸을 오른쪽으로 정렬, 빈칸 0 추가
...
SELECT EMP_NAME, EMP_NO, TO_DATE( SUBSTR(EMP_NO, 1, 6) , 'YYMMDD')
FROM EMPLOYEE ;

SELECT EMP_NAME, EMP_NO, TO_DATE( SUBSTR(EMP_NO, 1, 6) , 'RRMMDD')
FROM EMPLOYEE ;

SELECT TO_DATE('19491119', 'RRRRMMDD') FROM DUAL;
...
-- SELECT '1,000,000' + 10 FROM DUAL; -- 계산 안됨
...
-- EMPLOYEE 테이블에서 직원명, 직급코드, 급여, 인상급여를 조회
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D6'
ORDER BY JOB_CODE ASC; -- 정렬 기준이 되는 컬럼은 반드시 SELECT 절에 포함될 필요 X
...
-- MIN(컬럼명) : 최소값
-- MAX(컬럼명) : 최대값
--> 타입 제한 없음(숫자 : 숫자 중에 큰거/작은거, 날짜 : 과거/미래, 문자열 : 문자 순서)
--> 숫자, 날짜, 문자열 다 넣을 수 있음
...
-- 이 때, 결과는 각 그룹함수 별 독립된 결과로 나옴

-- 가장 높은 급여, 가장 늦은 입사일, 알파벳 순서가 가장 늦은 이메일, 사번이 200번이 아닌 사람 조회
...
--> 7행의 결과가 있지만 1행이 NULL이기 때문에 COUNT 시 제외되어 6행이 조회됨
