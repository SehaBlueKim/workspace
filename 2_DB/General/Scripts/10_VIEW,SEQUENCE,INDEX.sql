...
--> 각자 이니셜 계정에 VIEW 생성 권한을 부여해줄건데 (관리자 계정으로 접속)
GRANT CREATE VIEW TO KH_KSH;
...
-- DML을 쓸 수 있는데 왜 조회하는 용도로만 쓰는걸까? 알아보자
...
-- NOT NULL 제약조건 추가 후 VIEW를 이용한 INSERT 다시 수행 시 아래와 같은 오류 발생 
-- ORA-01400: NULL을 ("KH_KSH"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다

--> VIEW를 이용한 INSERT 시 원본 테이블에 삽입됨
--> 이 때, VIEW에 작성된 INSERT 구문에서 미포함된 컬럼에는 NULL이 저장되고
--> 만약 NOT NULL 제약조건이 설정된 컬럼이라면 오류가 발생한다
...
WITH READ ONLY; -- 읽기 전용의 VIEW 생성(DML 사용 못함)
...
-- EMPLOYEE_COPY4 의 EMP_ID 컬럼에 PK 제약조건 추가
...
START WITH 227 INCREMENT BY 5;

-- NEXTVAL 호출 없이 CURRVAL 호출하면 어떻게 되는지 보자 -> 오류 발생한다
SELECT SEQ_EMP_ID.CURRVAL FROM DUAL;
-- ORA-08002: 시퀀스 SEQ_EMP_ID.CURRVAL은 이 세션에서는 정의 되어 있지 않습니다

-- NEXTVAL 호출1
SELECT SEQ_EMP_ID.NEXTVAL FROM DUAL; -- 최초 호출 == START WITH 값 조회됨
-- 227

-- NEXTVAL 호출2
SELECT SEQ_EMP_ID.NEXTVAL FROM DUAL; -- INCREMENT BY 만큼 증가한 값 조회됨
-- 232
...
ALTER SEQUENCE SEQ_EMP_ID INCREMENT BY 3;
