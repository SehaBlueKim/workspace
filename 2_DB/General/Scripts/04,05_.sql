...
-- 2. 휴학중인 학생들의 이름과 주민번호를 나이가 적은 순서로 화명네 출력하시오
...
SELECT STUDENT_NAME , SUBSTR(STUDENT_NO ,1,8) , STUDENT_ADDRESS 
FROM TB_STUDENT 
...
--	  작성하시오. (법학과의 '학과코드'는 학과 테이블을 조회해서 찾아내도록 하자)
...
-- 교수 이름, 코드
SELECT PROFESSOR_NAME , NVL(COACH_PROFESSOR_NO, '지도교수 미지정') 지도교수
FROM TB_STUDENT 
...
-- "전공평점"으로 표시되도록 하고, 평점은 소수점 한자리까지만 반올림하여 표시되도록 한다.

-- 환경조경학과와 같은 계열인 인문사회 학과들 출력
SELECT DEPARTMENT_NAME
FROM TB_DEPARTMENT 
