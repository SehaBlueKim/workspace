...
-- 서로 같은 부서 코드를 나타낸다. (다만 부르는 컬럼명이 다를 뿐)
--> 이를 통해 두 테이블이 관계가 있음을 알고, 조인을 통해서 데이터 추출이 가능하다
...
--						  왼편에 기술된 테이블의 컬럼 수를 기준으로 JOIN 한다

-- ANSI
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) ;
--> LEFT JOIN : JOIN 구문을 기준으로 왼쪽에 작성된 테이블의 모든 행이
--				JOIN 결과에 포함되게 하는 JOIN 이다.
...
--						   오른편에 기술된 테이블의 컬럼 수를 기준으로 JOIN 한다
...
-- 사원의 급여가
-- SAL_LEVEL에 작성된 최소(MIN_SAL)부터 최대(MAX_SAL)
-- 범위의 급여가 맞을 때에만 JOIN
...
-- EMPLOYEE랑 DEPARTMENT 먼저 합치고, 그 합친 테이블을 LOCATION 테이블과 또 합침
...
-- 연봉에 보너스포인트를 적용하시오.
SELECT EMP_NAME, JOB_NAME, SALARY, SALARY * (NVL(BONUS,0)+1) * 12 연봉
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE)
