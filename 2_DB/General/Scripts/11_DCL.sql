...
--> 왜? 변경하지 않으면 c##을 사용자명 앞에 붙여야함... 번거롭다 (그리고 안이쁘다)
ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE;

-- 1. [SYS 관리자 계정] 사용자 계정 생성
-- 사용자명 : 각자 이니셜_sample / 비밀번호 sample1234
CREATE USER ksh_sample IDENTIFIED BY sample1234;

-- 2. 생성된 계정에 접속하기 위한 접속방법 추가
-- -> 데이터베이스 -> 새 데이터베이스 연결

-- ORA-01045: 사용자 KSH_SAMPLE는 CREATE SESSION 권한을 가지고있지 않아서 로그인 실패
...
--> DB 접속 권한만 있는 상태, 테이블 생성 권한은 없다

-- 6. [SYS 관리자 계정] 테이블 생성 권한 부여 + 객체 생성 공간도 할당
...
-- ROLE : 권한의 묶음
--> 묶어둔 권한(ROLE)을 특정 계정에 부여할 수 있음 --> 부여받은 계정은 특정 권한을 가진 역할을 갖게 됨
...
-- [SYS 관리자 계정]
SELECT * FROM ROLE_SYS_PRIVS -- PRIVS : PRIVILEGE 권한들을 줄여놓은 것
WHERE ROLE = 'CONNECT';
...
-- [SYS 관리자 계정]
SELECT * FROM ROLE_SYS_PRIVS -- PRIVS : PRIVILEGE 권한들을 줄여놓은 것
WHERE ROLE = 'RESOURCE'; -- 테이블 생성 포함 8개가 묶여져 있음
...
-- 객체 권한
--> ksh_sample 계정에서 KH_KSH 계정의 EMPLOYEE 테이블 조회

-- 1. [샘플 계정] KH_KSH 계정의 EMPLOYEE 테이블 조회
