...
-- <컬럼 값 산술 연산>
-- 컬럼 값 : 테이블의 한 칸(==한 셀)에 적용된 값(즉, data)
...
-- DUAL(DUmmy tAbLe) : 가짜 테이블(임시 테이블, 단순 조회 용도의 테이블)
...
-- SELECT 조회 결과의 집합인 RESULT SET에 컬럼명을 지정해줄 수 있다

/* 1) 컬럼명 AS 별칭 : 띄어쓰기X, 특수문자X, 그냥 문자O
 * 2) 컬럼명 별칭 : 1)번에서 AS만 생략
 * 
 * 3) 컬럼명 AS "별칭" : 띄어쓰기O, 특수문자O, 그냥 문자O
...
FROM EMPLOYEE 
ORDER BY SALARY ASC; -- ASCENT 오름, 상승

-- EMPLOYEE 테이블에서 급여 "내림차순"으로 이름, 급여 조회
SELECT EMP_NAME, SALARY
FROM EMPLOYEE 
ORDER BY SALARY DESC; -- DESCENT 내림, 하강
...
FROM EMPLOYEE
-- WHERE 연봉 >= 50000000 --> 별칭 해석 전이기 때문에 오류 발생 (해석 순서를 따져보면 됨)
-- ORDER BY 연봉 DESC; -- 별칭
...
SELECT EMP_NAME, BONUS
FROM EMPLOYEE
-- ORDER BY BONUS; -- NULLS LAST 기본값 (NULL을 마지막에 출력)
-- ORDER BY BONUS DESC; -- NULLS FIRST 기본값 (NULL을 먼저 출력)
