...
--> NOT NULL을 제외한 제약 조건들은 추가적인 조건으로 인식됨(ADD/DROP 사용)
--> NOT NULL은 기존 컬럼의 성질을 변경하는 것으로 인식됨(MODIFY 사용)

ALTER TABLE DEPARTMENT_COPY MODIFY DEPT_TITLE NOT NULL; -- NOT NULL은 MODIFY 사용!!
...
-- ALTER TABLE DEPARTMENT_COPY DROP CONSTRAINT SYS_C007504;
ALTER TABLE DEPARTMENT_COPY MODIFY DEPT_TITLE CONSTRAINT SYS_C007504 NULL;
...
ALTER TABLE DEPARTMENT_COPY ADD(LO_NAME VARCHAR2(30) DEFAULT '한국');
SELECT * FROM DEPARTMENT_COPY ; -- LO_NAME 컬럼 추가, 컬럼 값 모두 '한국'으로 됨
...
SELECT * FROM DEPARTMENT_COPY ; -- 인사관리부 == 15BYTE

-- (기본값 수정)
-- LO_NAME 기본값 '한국' -> '대한민국'으로 변경
ALTER TABLE DEPARTMENT_COPY MODIFY LO_NAME DEFAULT '대한민국';
SELECT * FROM DEPARTMENT_COPY ; --> 기본값을 변경했지만 기존에 저장된 값이 변경되지는 않음
...
ALTER TABLE DEPARTMENT_COPY DROP(DEPT_ID); -- 삭제 실패
-- ORA-12983: 테이블에 모든 열들을 삭제할 수 없습니다

ROLLBACK; -- 트랜젝션(DML을 이용한 데이터 변경 사항을 저장)을 삭제하고 마지막 커밋 상태로 돌아감

SELECT * FROM DEPARTMENT_COPY ;
--> ROLLBACK 안됨!
--> 왜?? CREEATE/ ALTER/ DROP 과 같은 DDL은 ROLLBACK의 대상이 아님!!

INSERT INTO DEPARTMENT_COPY VALUES('D0'); -- D0 삽입
SELECT * FROM DEPARTMENT_COPY ;
ROLLBACK; -- 트랜젝션에서 'D0' INSERT 구문 삭제
SELECT * FROM DEPARTMENT_COPY ;
...
-- DML을 수행하여 트랜젝션에 변경사항이 저장된 상태에서 (아직 DB로 넘어가지 않고 장바구니에 담긴 상태)
-- COMMIT/ROLLBACK 없이 DDL 구문을 수행하게 되면
-- DDL 수행 동시에 선행된 DML이 자동으로 COMMIT이 되어버린다.
...
-- 테이블명 DEPT_COPY -> DCOPY로 변경
ALTER TABLE DEPT_COPY RENAME TO DCOPY;

SELECT * FROM DEPT_COPY; -- 이름이 변경되어 조회 불가
SELECT * FROM DCOPY;
