...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		// JDBC : java와 DB가 연결할 수 있게 해주는 java API
		
		// [1단계] : JDBC 객체 참조 변수 선언 (java.sql 패키지)
		
		Connection conn = null;
		// DB 연결 정보를 담은 객체
		// -> Java와 DB 사이를 연결해주는 일종의 통로(Stream과 비슷하게 생각하면 됨)
		
		Statement stmt = null;
		// Connection 객체를 통해
		// Java에서 작성된 SQL을 DB로 전달하여 수행한 후
		// 결과를 반환 받아 다시 Java로 돌아오는 역할의 객체
		// (데이터를 싣고 통로를 왔다갔다 하는 셔틀버스 같은 애)
		
		ResultSet rs = null;
		// SELECT 질의 성공 시 반환되는
		// 결과 행의 집합(Result Set)을 나타내는 객체
		
		// 준비 끝. 이제 외부랑 DB랑 연결해줄거임
		...
		try {
			...
			conn = DriverManager.getConnection(type + ip + port + sid, user, pw);
								...
								// 드라이버 매니저가 커넥션을 얻어올건데, 어느 DB랑 연결해줄건지 알아야 연결해주겠죠?
								// 그래서 getConnection은 무조건 매개변수가 필요함
			
			// 중간확인
			// System.out.println(conn); // oracle.jdbc.driver.T4CConnection@4961f6af
			
			// 3. Statement 객체에 적재할 SQL문 작성하기 (셔틀버스에 태울 sql 승객 작성)
			// ORA-00933: SQL 명령어가 올바르게 종료되지 않았습니다
			// *** Jave에서 작성된 SQL문은 마지막에 ;(세미콜론)을 찍으면 안됨!! ***
			String sql = "SELECT EMP_ID, EMP_NAME, SALARY, DEPT_CODE FROM EMPLOYEE";
			
			// 4. Statement 객체 생성
			// (Connection 이라는 도로 위를 달릴 셔틀버스 만들기)
			stmt = conn.createStatement();
			
			// 5. SQL을 Statement에 적재 후 DB로 전달하여 수행 한 후
			//	  결과를 반환 받아와 rs 변수에 대입 (rs에는 주소값이 담기게 됨)
