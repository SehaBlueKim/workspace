...
// View : 입,출력 담당 클래스
// - 사용자 담당 인터페이스 요소로 사용자의 요청과 응답을 보여주는 화면만 만들거임
public class EmployeeView {
	...
	private void deleteEmployee() {
		...
		// DML은 성공한 행의 개수를 반환해줌
		int result = service.deleteEmployee(input);

		if(result != 0) { // 조회 결과가 있다면
	...
	private void updateEmployee() {
		...
		else System.out.println("일치하는 사번의 사원이 없습니다.");
	}
	...
	private void updateBonus() {
		...
		Employee emp = new Employee(deptCode, bonus);
		
		int result = service.updateBonus(emp);
		
		if(result != 0) System.out.println(deptCode + " 부서의 보너스율이 " + bonus + "으로 변경되었습니다.");
		else System.out.println("일치하는 부서코드가 존재하지 않습니다.");
	}
