...
public class MainService {
	...
	public void signUp() { 
		...
		for(int i=0; i<members.size(); i++) {
			if(members.get(i).getMemberName().equals(id)) {
				flag = true;
				break;
			}
		}
	...
	public void login() {
		...
		for(int i=0; i<members.size(); i++) {
			if(members.get(i).getMemberId().equals(id)
					&& members.get(i).getMemberPw().equals(pw)) {
	...
	private void sortScore() {
		System.out.println("\n----- 랭킹 -----\n");
		List<Member> sortList = loginService.sortScore(members);
		for(Member m : sortList) {
