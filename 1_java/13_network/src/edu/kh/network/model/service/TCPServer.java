...
public class TCPServer {
   ...
   public void serverStart() {
      ...
      // 단, 1023번 이하는 이미 사용중인 경우가 많아서 제외

      // * 사용할 변수 미리 선언 *
      ServerSocket serverSocket = null; // 서버 소켓을 저장하기 위한 변수
      Socket clientSocket = null; // 클라이언트 소켓을 저장하기 위한 변수

      InputStream is = null; // 클라이언트 -> 서버 입력용 스트림 변수 (서버 입장에서 들어오는거)
      BufferedReader br = null; // 입력용 보조 스트림 변수

      OutputStream os = null; // 서버 -> 클라이언트 출력용 스트림 변수 (서버 입장에서 나가는거)
      ...
      try {
         ...
         // -> 요청을 수락하면 자동으로 클라이언트의 Socket 객체가 얻어와짐
      ...
      }finally { // try 구문에서 예외 발생 여부 관계없이 무조건 수행
         // 8. 통신 종료
         ...
         try { 
            // 보조스트림 close 시 연결된 기반 스트림(is, os)도 같이 close 됨
            if(pw!=null) pw.close(); // + os.close()
            if(br!=null) br.close(); // + is.close()
            if(clientSocket!=null) clientSocket.close(); // + is.close()
            if(serverSocket!=null) serverSocket.close(); // + is.close()
         } catch (IOException e) {
            e.printStackTrace();
         }
      }
