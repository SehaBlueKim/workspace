...
public class MapEx {
	 ...
	 *  * Key 역할을 하는 객체에는 equals(), hashCode()가 오버라이딩 되어 있어야 함
	 * 
	 * 2) 순서 유지 X
	 * 
	...
	public void ex1() {
		...
		// Map에 추가 시 Key가 중복되면 새로운 Value로 덮어 씌우고 이전 Value가 반환
		// 중복이 아니면 null 반환
	...
	public void ex3() {
		...
		// 1) 한번에 많은 양의 데이터를 명확하게 구분할 수 있는 형태로 전달해야 하는 경우
		...
		// == 어떤 객체든 Value에 들어올 수 있다! (Object는 최상위 클래스니까)
		
		s1.put("grade", 3); // 3(int) -> Integer로 Auto Boxing(Wrapper Class)
		s1.put("ban", 5);
		s1.put("number", 17);
		s1.put("name", "카리나");
		s1.put("gender", 'F');
