...
public class StudentView {
	...
	public void selectAllStudent() {
		...
		// service로 부터 List<Student> 객체의 참조 주소를 반환 받은 후
		// studentList에 얕은 복사
	...
	private void selectOne() {
		...
		// std = null 또는 학생 1명(Student) 객체 주소가 저장되어있는 상태
	...
	private void selectGender() {
		...
		// 만약 검색 결과가 없을 경우
		// == searchList가 비어있을 경우
		// == searchList의 size()가 0인 경우 (아무것도 저장되지 않았음)
	...
	private void selectGrade() {
		...
		if(searchList.isEmpty()) { // 검색 결과 없을 경우
			System.out.println("검색 결과가 존재하지 않습니다.");

		} else { // 존재하는 경우 해당 학년의 학생 목록 조회 후 출력
			for(Student std : searchList) {
				System.out.println(std);
			}
		}
	...
	private void selectName() {
		...
		if(searchList.isEmpty()) {
			// 검색 결과 없을 경우 "검색 결과가 존재하지 않습니다." 출력
			System.out.println("검색 결과가 없습니다.");
	...
	private void duplicateValidation() {
		...
		// 입력 받은 값을 서비스로 전달하여 중복이면 true, 아니면 false 반환 받기
