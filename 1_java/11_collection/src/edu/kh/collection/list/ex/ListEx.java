...
public class ListEx {
	...
	public void ex1() {
		// 다형성 적용
		// List list = new List(); 
		// 오류 원인 : List는 인터페이스임. 따라서 미완성된 설계도로 객체 생성 불가
		List list = new ArrayList(3); // 3칸 짜리 생성
		// List list = new ArrayList(); // 10칸 짜리 생성
		
		// 1. boolean add(E e) : 마지막 요소로 추가
		// E(Element) : 요소를 뜻하는 상징적인 글자일 뿐 (자료형 아님)
		// -> Object를 생각하면 편함
		list.add("아무거나"); // String은 객체
		
		// Auto Boxing (자동으로 포장해줌)
		list.add(123); // int(기본 자료형) --자동포장--> Integer(객체)
		list.add(3.14); // double(기본 자료형) --자동포장--> Double(객체)
		...
		for(int i=0; i<list.size(); i++) {
			...
			if(list.get(i) instanceof Integer) { // Integer인 경우
				...
				// Auto Unboxing 자동으로 실행되어
				// Integer를 int 에 대입하는 것이 가능
				int number = (Integer)list.get(i);
				System.out.println(200+number);
			}
			
			if(list.get(i) instanceof Double) { // Double인 경우
				Double d = (Double)list.get(i); // 다운캐스팅만 진행, Auto Unboxing 안함
	...
	public void ex2() {
		...
		// E list.remove(int index) : index번째 요소를 List에서 제거
		// 반환되는 값은 제거된 요소
		String target = list.remove(6);
		System.out.println(target + " 제거");
		
		// boolean list.contains(E e) : list에 요소가 포함되어 있는지 확인
		System.out.println(list.contains("닭갈비"));
		System.out.println(list.contains("파닭"));
		
		// int list.indexOf(E e) : list에 요소가 존재하면 해당 index 반환
		// 없으면 -1 반환
		...
		// boolean list.isEmpty() : 비어있으면 true, 아니면 false
