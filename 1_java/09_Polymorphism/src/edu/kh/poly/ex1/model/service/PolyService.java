...
public class PolyService {
	...
	public void ex1() {
		...
		car.setSeat(6);
		car.setFuel("휘발유");
		// Car 메소드 모두 사용 가능 (내꺼 내가 쓰겠다는데 문제있어?)
		...
		// 각 배열 요소가 Car 타입 참조 변수다.

		arr[0] = car; // Car 주소 (== Car 객체)
		// Car타입 참조변수

		arr[1] = car2; // Porsche 주소 (== Prosche 객체)
		// Car타입 참조변수
		...
		//			-> Car를 상속 받은 클래스는 모두 getWheel()을 가지고 있다를 정의
	...
	public void ex2() {
		...
		// -> 참조하는 객체가 특정 자료형이거나 부모쪽 상속 관계인지를 확인함

		// arr[1]이 참조하는 객체가 Porsche라면 true, 아니면 false
		System.out.println(arr[1] instanceof Porsche); // true

		// arr[1]이 참조하는 객체가 Spark라면 true, 아니면 false
		System.out.println(arr[1] instanceof Spark); // false

		// arr[1]이 참조하는 객체가 Car라면 true, 아니면 false
	...
	public void ex5() {
		...
		// -> "실행 시"에는 자식(Spark)의 오버라이딩된 메소드와 바인딩이 일어남 == [동적 바인딩]
	...
	public void printCar(Car poly) {
		// 매개변수에 작성된 참조형 변수에는 주소가 저장된다. (== 얕은 복사가 일어난다)
