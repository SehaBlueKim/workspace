$(document).ready(function(){ // document가 준비되면 이 함수를 실행하겠다
    // id가 test인 요소의 테두리를 2px 빨간 실선, 글자를 파란색으로 변경
    // $("#test").css("border", "2px solid red").css("color", "blue");

    // 자바스크립트 객체를 매개변수로 전달
    // js객체의 key는 무조건 string이기 때문에 "", '' 또는 생략 가능하다.
...
// 클래스가 wrap인 요소의 후손 ul, p태그의 스타일을 style2로 설정
$(".wrap").children().children().css(style2);

// 클래스가 type인 요소의 자식 중에서 ul태그의 스타일을 style3로 설정
...
// 클래스가 wrap인 요소의 후손 중에서 모든 span 태그의 스타일을 style5로 설정
