...
/* 
    ...
    정적 요소 : HTML 문서 로딩 전부터 이미 작성되어 있는 요소
        -> 기존 방법 on("click", function(){})

    동적 요소 : 자바스크립트 또는 제이쿼리에 의해서
                HTML 문서 로딩 이후 추가되는 요소들한테는
            -> 기존 방법 적용 불가하다
            -> $(document).on("이벤트", "선택자", function(){})
            (문서에서 click이라는 이벤트가 발생했을 때 
            대상이 선택자에 작성된 애라면 function을 수행하겠다 라는 의미)
*/
...
// 콘솔에 "입력값이 블럭이 잡힘." 출력
// 블럭값이 잡힘 -> 마우스로 드래그 했을 때 블럭이 쳐지는걸 말하는거
...
$("#input-content").on("input", function(){
    ...
    if(length<= 130){
        ...
        // 2. substring을 이용해서 글자를 150자까지만 잘라내기
        // 문자열.substring(시작인덱스, 종료인덱스[미포함])
        str.substring(0, 150)

        // 3. 150글자만 잘라내어 textarea의 값으로 세팅
        $(this).val( str.substring(0, 150) );
        // 151번째 입력이 인식되면 150글자로 자름 
        // 그런데 자르기 전에 글자수를 카운트해서 151로 보이는 문제가 발생
