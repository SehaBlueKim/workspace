...
// ready() 함수 : 문서가 로딩된 후 가장 마지막에 수행하는 함수
$(document).ready(function(){ // 문서가 준비되면 이 함수를 수행하겠다
    ...
    // -> 배열에 요소를 하나씩 꺼내서 적용하는건 가능하나 불편함
    // -> 간소화를 목적으로 만들어진 jQuery로 훨씬 간편하게 쓸 수 있음
...
$("#input1").on("input", function(){
    ...
    if(regExp.test($("#input1").val())){
        // $("#input1").val()
        // -> 아이디가 input1 인 요소에 작성된 값(즉, value)를 얻어옴
        ...
        // method chaining : 하나의 대상에 대하여 여러 메소드를ㄹ 연달아 작성하는 기술
    }
})
...
$("#check").on("click", function(){
    ...
    // name 속성값이 gender인 요소 중 check 된 요소만 선택
    // : checked -> 체크된 요소만 선택
    const gender = $("input[name='gender']:checked");
    console.log(gender.length);
    // 아무것도 check 안한 경우 : 0 반환
    // check 한 경우 : 1 반환
    ...
    if(gender.length == 0){
        ...
        // 1) 체크된 요소를 얻어왔으므로
        //    gender 변수에서 value만 얻어오기 (순수 JS만 이용)
        console.log(gender[0].value);

        // 2) 체크된 요소를 모두 얻어와도
        //    radio는 한개만 체크되기 때문에
        //    변수 한개랑 같다고 해석하여
        //    자동으로 0번 인덱스에 있는 요소의 value를 얻어온다(JS + jQuery)
        console.log(gender.val());

        // 3) 순수 jQuery를 이용해서 얻어와보기
        console.log($(gender).val());
        // $(gender) : 체크된 요소만 담긴 배열 + 해당 요소를 선택
