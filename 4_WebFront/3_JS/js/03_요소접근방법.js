...
function accessClass(){
    ...
    arr[1].style.backgroundColor = "cornflowerblue";
...
function inputTest(){
    ...
    // ** input은 [value]를 이용해서 값을 얻어오거나, 변경할 수 있다

    // input에 작성된 값 변경하기
    input.value = ''; // 빈 문자열 == value 지우기 용도로 쓸 수 있음
...
function readValue(){
    ...
    if(input.value.trim() != ''){
        ...
        // 요소.scrollHeight     : 스크롤 전체 높이 구하기

        // bg의 스크롤을 제일 밑으로 내리기
        bg.scrollTop = bg.scrollHeight;
    }

    // input에 작성된 값 변경(비워주기)
    input.value = '';
    // input에 초점 맞추기
    input.focus();
}
...
function inputEnter(event){
    ...
    if(event.key == "Enter"){ // 눌러진 키가 Enter인 경우
        readValue();
    }
}
